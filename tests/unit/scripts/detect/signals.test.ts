...
describe("framer matchers", () => {
  it("matches meta generator=Framer <7hex> in either attribute order", () => {
    const orderA = view({
  ...
  it("matches the Built with Framer / Made in Framer HTML comment", () => {
    const built = view({ homeHtml: "<!-- Built with Framer --><html><body></body></html>" });
    const made = view({ homeHtml: "<!-- Made in Framer --><html><body></body></html>" });
    expect(find(framerSignals, "fr-comment-madein").match(built)).not.toBeNull();
    expect(find(framerSignals, "fr-comment-madein").match(made)).not.toBeNull();
  });
  ...
  it("matches the Framer server response header", () => {
  ...
  it("counts >=5 data-framer-name/component-type occurrences as one hit", () => {
    const below = view({
      homeHtml: `<html><body>${'<div data-framer-name="a"></div>'.repeat(4)}</body></html>`,
    });
    const atThreshold = view({
      homeHtml: `<html><body>${'<div data-framer-name="a"></div>'.repeat(5)}</body></html>`,
    });
  ...
  it("counts >=10 framer-<hash> classes as one hit", () => {
    const below = view({
      homeHtml: `<html><body class="${Array.from({ length: 9 }, (_, i) => `framer-abcd${i}`).join(" ")}"></body></html>`,
    });
    const atThreshold = view({
      homeHtml: `<html><body class="${Array.from({ length: 10 }, (_, i) => `framer-abcd${i}`).join(" ")}"></body></html>`,
    });
  ...
  it("matches the server-timing header with ssg-status and a route id", () => {
    const v = view({
      homeHttp: http({
        headers: { "server-timing": 'ssg-status;desc="static", route;dur=1;desc="id=abc"' },
      }),
    });
    expect(find(framerSignals, "fr-header-server-timing").match(v)).not.toBeNull();
  });

  it("produces ZERO hits for a page that merely mentions framer in text", () => {
    const v = view({
      homeHtml:
        '<html><head></head><body><p>We migrated away from Framer to Payload last year.</p><a href="/vs/framer">Framer vs us</a></body></html>',
    });
