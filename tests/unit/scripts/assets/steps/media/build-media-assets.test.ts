...
import { A1, A2, cdn, makeFakeStore, nullLogger, type FakeStoreConfig } from "../../fixtures/assets.ts";
...
describe("buildMediaAssets", () => {
  ...
  it("keeps only the original and skips scale-down-to variants", async () => {
    const original = cdn(`/images/${A1}.jpg`);
    const html = `<img src="${original}" srcset="${original}?scale-down-to=512&width=1024&height=683 512w, ${original}?scale-down-to=1024&width=1024&height=683 1024w">`;
    const data = await build({
  ...
  it("folds a bare original and its intrinsic-size query onto one canonical asset", async () => {
    const canonical = cdn(`/images/${A1}.svg`);
    const data = await build({
      pages: [
        {
          url: "https://site.example/",
          html: `<img src="${canonical}?width=200&height=200"><img src="${canonical}">`,
        },
      ],
  ...
  it("marks the second asset with identical bytes as an alias of the first", async () => {
    const u1 = cdn(`/images/${A1}.jpg`);
    const u2 = cdn(`/images/${A2}.jpg`);
