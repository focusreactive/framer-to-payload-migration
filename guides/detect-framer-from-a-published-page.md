...
- **`framerusercontent.com` as the origin of every asset.** Every image, font and video a Framer
  ...
  three or more times, or seeing the `/sites/<id>/` path specifically, is strong evidence — but a
  site can self-host copies of Framer-exported assets without being a Framer site, so this signal
...
(`data-framer-ssr-released-at`, `data-framer-page-optimized-at`, `data-framer-root`), and CSS/class
fingerprints (`framer-<hash>` utility classes, `--token-<uuid>` design-token custom properties,
`app.framerstatic.com` font URLs).

## How much evidence is enough?

Each signal carries a tier, and each tier a weight: `strong` = 3, `medium` = 2, `weak` = 1. A page
needs a total of **6** to be called Framer.

Two rules keep that number honest.

**Families are capped.** Related signals are grouped — the SSR container's attributes, the CDN
markers, the CSS fingerprints, the `data-framer-*` attribute family — and each group contributes at
most 6 regardless of how many of its members hit. Without the cap, a page carrying a handful of
`data-framer-*` attributes for unrelated reasons could clear the threshold on one family alone.
Capping forces corroboration from an unrelated kind of evidence.
...
the strong signals carry that flag — or the one `instant` signal, a host ending in
`.framer.website`, `.framer.app`, `.framer.photos`, `.framer.media` or `.framer.wiki`, which alone
satisfies the threshold because those domains cannot serve anything but a Framer site. The `server`
header, meanwhile, is real evidence but is not itself HTML-observable, so it cannot carry a verdict
by itself — a page that failed every HTML check but had a spoofed `server` header should not pass.
